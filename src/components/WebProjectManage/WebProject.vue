<template>
    <section id="webProject">
        ...
        <el-row class="toolbar">
            <el-button type="primary" icon="el-icon-circle-plus" @click="openWebProjectSave()">添加</el-button>
            <!-- 搜索模块 -->
        ...
        <!-- 保存弹窗 -->
        <WebProjectSave ref="projectSave"></WebProjectSave>
    </section>
</template>
...
<script>
    ...
    import WebProjectSave from './WebProjectSave';
    ...
    export default {
        ...
        methods: {
            ...
            deleteWebProject(row) {
                console.log(row);
                this.$confirm('确定要删除[' + row.projectCode + ']该条记录?', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    $http.get(api.coreDeleteWebProject).then(res => {
                        console.log(res.data);
                        this.$message({
                            type: 'success',
                            message: '项目[' + row.projectCode + '] 删除成功!'
                        });
                        this.loadData();
                    });
                }).catch(() => {
            ...
            openWebProjectSave() {
                this.$refs['projectSave'].openDialog();
            },
            editWebProject(row) {
                console.log(row);
                this.$refs['projectSave'].getProjectDetail(row.id);
                this.openWebProjectSave();
            }
        }
    }
    ;
